...
namespace LoyaltySurvey.Pages.Helpers {
	public partial class PageTemplate : Page {
		...
		protected double LeftCornerShadow { get; set; } = 0;
		protected double RightCornerShadow { get; set; } = 5;
		private double _elementsInLine = 0;
		//private double _elementsLineCount = 0;
		private double _elementsGap = 0;

		protected ItemSurveyResult SurveyResult { get; set; } = null;
		...
		private void CreateMainControls() {
			...
			_ = PageControlsFactory.CreateImage(
...
#pragma warning restore CA1303 // Do not pass literals as localized parameters
		...
		protected Rect CreateFirstOrLastPageControls(string leftTopText,
			...
			if (!string.IsNullOrEmpty(subtitleText)) {
				rect.Height -= DefaultButtonHeight;
				PageControlsFactory.CreateLabel(
					subtitleText,
					Colors.Transparent,
					Properties.Settings.Default.ColorDisabled,
		...
		protected void CreateRootPanel(double elementsInLine,
								 double elementsLineCount,
			...
			if (type == PageControlsFactory.ElementType.Department &&
				...
				_elementsGap = RightCornerShadow;
			...
			foreach (string element in elements) {
				...
				if (linesCreated == totalLines - 1) {
					...
					if (totalLines > 1 && !isLastLineCentered) {
						...
						leftMargin = (ScrollViewer.Width - RightCornerShadow - lastElementsWidth) / 2.0d;
		...
		protected static void WriteSurveyResultToDb(ItemSurveyResult surveyResult) {
		...
		protected void AutoCloseAllPages(bool showDepartmentSelect = false) {
			if (SurveyResult != null) {
